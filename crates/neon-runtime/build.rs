...
fn main() {
    let electron_version = {
        let electron_mode = match std::env::var("CARGO_FEATURE_ELECTRON") {
            Ok(r) => r == "1",
            Err(_) => false,
        };
        ...
        if electron_mode {
            ...
            .args(&[ "-v" ])
            .output()
            .expect("No 'electron' command in your PATH.");

            let version = String::from_utf8_lossy(&output.stdout).replace("v", "").replace("\n", "");
            Option::Some(version.to_owned())
        } else {
            Option::None
        }
    };
    ...
    build_object_file(electron_version);
...
fn build_object_file(electron_version: Option<String>) {
    if cfg!(windows) {
    ...
    let mut cmd = npm();
    let cmd_npm = {
        let cmd = cmd.args(&["run", if debug() { "configure-debug" } else { "configure-release" }]);
        match electron_version.as_ref() {
            Some(version) => cmd.env("NEON_RUNTIME_ARGS", format!("--target={} --dist-url=https://electronjs.org/headers", version.clone())),
            None => cmd,
        }
    };
    ...
    let mut cmd = npm();
    let cmd_npm = {
        let cmd = cmd.args(&["run", if debug() { "build-debug" } else { "build-release" }]);
        match electron_version.as_ref() {
            Some(version) => cmd.env("NEON_RUNTIME_ARGS", format!("--target={} --dist-url=https://electronjs.org/headers", version.clone())),
            None => cmd,
        }
